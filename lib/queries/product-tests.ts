...
export async function getTestStatistics(productId: string): Promise<TestStatistics> {
    ...
    // Critical failures (join with product_specs to check is_critical)
    ...
    const previousTests = allTests?.filter(t => new Date(t.tested_at) >= sixtyDaysAgo && new Date(t.tested_at) < thirtyDaysAgo) || [];
    ...
    if (recentPassRate > previousPassRate + 5) recent_trend = 'improving';
    else if (recentPassRate < previousPassRate - 5) recent_trend = 'declining';
...
export async function getOutOfSpecTests(
    ...
    if (dateFrom) query = query.gte('tested_at', dateFrom);
    if (dateTo) query = query.lte('tested_at', dateTo);
...
export async function getParameterTrend(
    ...
    const average = numericValues.reduce((a, b) => a + b, 0) / (numericValues.length || 1);
