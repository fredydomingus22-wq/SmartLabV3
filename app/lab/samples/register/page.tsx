// Sample Registration Page - Refactored with validation and UI enhancements
...
export default function SamplesRegistrationPage() {
    ...
    useEffect(() => {
        if (sampleType === "intermediate_product") setPhase("intermediate");
        else if (sampleType === "finished_product") setPhase("finished");
    }, [sampleType]);
    ...
    // Reload recent samples when filters change
    useEffect(() => {
        loadRecentSamples();
    }, [filters]);

    // Load specifications when lot & phase are selected
    useEffect(() => {
    ...
    async function loadData() {
        ...
        try {
            ...
            const [productionLotsData, { data: intermediateLotsData, error: intermediateLotsError }, { data: tanksData, error: tanksError }] = await Promise.all([
                getActiveProductionLots(),
                supabase.from("intermediate_lots").select(`
          id,
          code,
        ...
        `).eq("status", "active").order("created_at", { ascending: false }),
                supabase.from("tanks").select("id, code, name").eq("active", true).order("code"),
            ]);
    ...
    function generateCodePreview() {
        ...
        const preview = previewSampleCode(productCode, selectedLot.code, tankCode);
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            ...
            if (needsProduct) {
                const { data, error: codeError } = await supabase.rpc("generate_sample_code", {
                ...
                codeData = { code: `${sampleType.substring(0, 3).toUpperCase()}-${timestamp}`, sequence: 0 };
            }
            ...
            const { error: sampleError } = await supabase.from("samples").insert(validation.data).select().single();
            ...
            <AppShell>
                <div className="p-6">
                    <Alert variant="destructive">
                        ...
                        <AlertDescription>Não tem permissão para registar amostras.</AlertDescription>
                    </Alert>
                </div>
            </AppShell>
        );
    ...
    const availableLots = lots.filter((lot) => (phase === "intermediate" ? lot.type === "intermediate" : lot.type === "production"));

    return (
        <AppShell>
            <div className="p-6 space-y-6">
                <SectionHeader title="Registro de Amostras" description="Registar novas amostras de produtos, swabs e análises ambientais" />
                <div className="grid gap-6 lg:grid-cols-3">
                    ...
                    <Card className="lg:col-span-2">
                        <CardHeader>
                            ...
                            <CardDescription>Preencha os dados para registar uma nova amostra</CardDescription>
                        </CardHeader>
                        <CardContent>
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                {sampleTypeConfig && (
                                    <Alert>
                                        ...
                                        <AlertDescription className="text-xs">
                                            {sampleTypeConfig.needsLot ? "Requer seleção de lote de produção e tanque" : "Não requer lote de produção"}
                                        </AlertDescription>
                                    </Alert>
                                )}
                                {/* Conditional fields for product samples */}
                                {needsProduct && (
                                    <>
                                        {/* Phase Selection */}
                                        <div className="space-y-2">
                                            <Label htmlFor="phase">Fase do Produto *</Label>
                                            <Select value={phase} onValueChange={(value) => setPhase(value as "intermediate" | "finished")}
                                            ...
                                            <Select value={selectedLotId} onValueChange={setSelectedLotId}>
                                                ...
                                                <SelectContent>
                                                    {availableLots.length === 0 ? (
                                                        <div className="p-2 text-sm text-muted-foreground text-center">Nenhum lote disponível para esta fase</div>
                                                    ) : (
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <Select value={selectedTankId} onValueChange={setSelectedTankId}>
                                                ...
                                                <SelectContent>
                                                    {tanks.length === 0 ? (
                                                        <div className="p-2 text-sm text-muted-foreground text-center">Nenhum tanque ativo disponível</div>
                                                    ) : (
                                                        tanks.map((tank) => (
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <Textarea id="observations" placeholder="Observações adicionais..." value={observations} onChange={(e) => setObservations(e.target.value)} rows={3} />
                                            {formErrors["observations"] && (
                                                <p className="text-sm text-red-500 mt-1">{formErrors["observations"]}</p>
                                            )}
                                        </div>
                                    </>
                                )}
                                ...
                                {!needsProduct && (
                                    <div className="space-y-2">
                                        <Label htmlFor="location">Local / Identificação *</Label>
                                        <Input
                                            id="location"
                                            placeholder={sampleType.includes("swab") ? "Ex: Linha 3, Ponto de Amostragem A" : "Ex: Torneira Principal, Lab Microbiologia"}
                                            value={swabLocation}
                                            onChange={(e) => setSwabLocation(e.target.value)}
                                        />
                                        <p className="text-xs text-muted-foreground">Descreva o local ou identificação da amostra</p>
                                    </div>
                                )}
                                    ...
                                    {specsLoading ? (
                                        ...
                                        <div className="space-y-3">
                                            <div className="flex items-center justify-between text-sm">
                                                ...
                                                <span className="font-medium">{specsInfo.phase === "intermediate" ? "Intermédio" : "Acabado"}</span>
                                            </div>
                                            <div className="flex items-center justify-between text-sm">
                                                <span className="text-muted-foreground">Parâmetros:</span>
                                                <span className="font-bold text-emerald-600">{specsInfo.parameter_count}</span>
                                            </div>
                                            <Alert className="bg-emerald-500/10 border-emerald-500/50">
                                                <CheckCircle className="h-4 w-4 text-emerald-600" />
                                                <AlertDescription className="text-xs">Especificações carregadas automaticamente</AlertDescription>
                                            </Alert>
                                        </div>
                                    ) : (
                                        <p className="text-sm text-muted-foreground text-center py-4">Selecione um lote e fase para ver as especificações</p>
                                    )}
                                </CardContent>
                            </Card>
                        )}
                            ...
                            <Card>
                                ...
                                <CardContent>
                                    <div className="space-y-2 text-xs text-muted-foreground">
                                        ...
                                        <code className="block bg-slate-900 p-2 rounded text-emerald-400">[ProductCode][4digitsLot]-[Tank]-[Seq]</code>
